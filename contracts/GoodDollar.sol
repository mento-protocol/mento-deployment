...
contract GoodDollar is UUPSUpgradeable, ERC677, ERC20PresetMinterPauserUpgradeable {
   ...
   */
  function transfer(address to, uint256 value) public override(ERC20Upgradeable, ERC677) returns (bool) {
  ...
  function transferAndCall(address to, uint256 value, bytes calldata data) external returns (bool) {
    ...
    if (isContract(to)) {
      require(contractFallback(to, bruttoValue, data), "Contract fallback failed");
    }
    return res;
  }
  ...
  function mint(address to, uint256 value) public override returns (bool) {
    if (cap > 0) {
      require(totalSupply() + value <= cap, "Cannot increase supply beyond cap");
    }
   ...
   */
  function getFees(uint256 value) public view returns (uint256 fee, bool senderPays) {
   ...
   */
  function _processFees(address account, address recipient, uint256 value) internal returns (uint256) {
    (uint256 txFees, bool senderPays) = getFees(value, account, recipient);
    if (txFees > 0 && !identity.isDAOContract(msg.sender)) {
      require(senderPays == false || value + txFees <= balanceOf(account), "Not enough balance to pay TX fee");
