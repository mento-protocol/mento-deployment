...
contract cINRChecksVerify is cINRChecksBase {
  ...
  function run() public {
    ...
    console.log("\nStarting cINR checks:");

    console.log("\n==  Rate feeds ==");
    console.log("   INRUSD: %s", config.rateFeedConfig.rateFeedID);
  ...
  function verifyToken(cINRConfig.cINR memory config) internal {
    console.log("\n== Verifying Token Config Transactions ==");
    verifyOwner();
    verifyCINRStableToken(config);
    verifycINRAddedToReserve();
    verifycINRAddedToFeeCurrencyWhitelist();
    verifyConstitution();
  }

  function verifyOwner() internal view {
    require(Proxy(cINR)._getOwner() == governance, "StableTokenINR Proxy ownership not transferred to governance");
    console.log("🟢 cINR proxy ownership transferred to governance");
  }

  function verifyCINRStableToken(cINRConfig.cINR memory config) internal {
    StableTokenINRProxy stableTokenINRProxy = StableTokenINRProxy(cINR);
    address stableTokenV2 = contracts.deployed("StableTokenV2");

    address cINRImplementation = stableTokenINRProxy._getImplementation();
    if (cINRImplementation != stableTokenV2) {
      console.log(
        "The implementation from StableTokenINRProxy(%s): %s does not match the deployed StableTokenV2 address: %s.",
        cINR,
        cINRImplementation,
        stableTokenV2
      );
      revert("StableTokenINRProxy does not point to StableTokenV2 deployed implementation. See logs.");
    }
    console.log("🟢 StableTokenINRProxy has the correct implementation address");

    // ----- verify initialization parameters -----
    IStableTokenV2 cINRToken = IStableTokenV2(cINR);
    IERC20Metadata cINRTokenMetadata = IERC20Metadata(cINR);

    assertEq(cINRTokenMetadata.name(), config.stableTokenConfig.name, "❗️❌ cINR name not set correctly!");
    assertEq(cINRTokenMetadata.symbol(), config.stableTokenConfig.symbol, "❗️❌ cINR symbol not set correctly!");
    assertEq(cINRToken.broker(), broker, "❗️❌ cINR broker not set correctly!");
    assertEq(cINRToken.validators(), validators, "❗️❌ cINR validators not set correctly!");

    // no pre-mint
    assertEq(cINRToken.totalSupply(), 0, "❗️❌ cINR pre-minted tokens!");
  }

  function verifycINRAddedToReserve() internal view {
    if (!Reserve(address(uint160(reserve))).isStableAsset(cINR)) {
      revert("cINR has not been added to the reserve.");
    }

    console.log("🟢 cINR has been added to the reserve");
  }

  function verifycINRAddedToFeeCurrencyWhitelist() internal view {
    address[] memory feeCurrencyWhitelist = IFeeCurrencyWhitelist(contracts.celoRegistry("FeeCurrencyWhitelist"))
      .getWhitelist();

    if (!Arrays.contains(feeCurrencyWhitelist, cINR)) {
      revert("cINR has not been added to the fee currency whitelist.");
    }

    console.log("🟢 cINR has been added to the fee currency whitelist");
  }
  ...
  function checkConstitutionParam(bytes4 functionSelector, uint256 expectedValue) internal view {
    uint256 actualConstitutionValue = celoGovernance.getConstitution(cINR, functionSelector);
  ...
  function verifyExchange(cINRConfig.cINR memory config) internal view {
    console.log("\n== Verifying exchanges ==");
  ...
  function verifyPoolExchange(cINRConfig.cINR memory config) internal view {
  ...
  function verifyPoolConfig(cINRConfig.cINR memory config) internal view {
    bytes32 exchangeId = getExchangeId(
      config.poolConfig.asset0,
      config.poolConfig.asset1,
      config.poolConfig.isConstantSum
  ...
  function verifyTradingLimits(cINRConfig.cINR memory config) internal view {
  ...
  function verifyCircuitBreaker(cINRConfig.cINR memory config) internal view {
  ...
  function verifyBreakersAreEnabled(cINRConfig.cINR memory config) internal view {
  ...
  function verifyMedianDeltaBreaker(cINRConfig.cINR memory config) internal view {
