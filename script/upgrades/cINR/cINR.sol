 ...
 */
contract cINR is IMentoUpgrade, GovernanceScript {
  using TradingLimits for TradingLimits.Config;
  using FixidityLib for FixidityLib.Fraction;

  ICeloGovernance.Transaction[] private transactions;

  address payable private stableTokenINRProxy;
  ...
  function loadDeployedContracts() public {
    ...
    contracts.load("cINR-00-Create-Proxies", "latest");
  }
  ...
  function setAddresses() public {
    // Tokens
    stableTokenINRProxy = contracts.deployed("StableTokenINRProxy");
  ...
  function buildProposal() public returns (ICeloGovernance.Transaction[] memory) {
    ...
    proposal_initializeCINRToken(config);
    proposal_configureCINRConstitutionParameters();
    proposal_addCINRToReserve();

    proposal_enableGasPaymentsWithCINR();
  ...
  /**
   * @notice Configures the cINR token
   */
  function proposal_initializeCINRToken(cINRConfig.cINR memory config) private {
    StableTokenINRProxy _cINRProxy = StableTokenINRProxy(stableTokenINRProxy);
    if (_cINRProxy._getImplementation() == address(0)) {
      transactions.push(
        ICeloGovernance.Transaction(
          0,
          stableTokenINRProxy,
          abi.encodeWithSelector(
            _cINRProxy._setAndInitializeImplementation.selector,
      ...
      console.log("StableTokenINRProxy is already initialized, skipping initialization.");
    }
  }

  /**
   * @notice configure cINR constitution parameters
   * @dev see cBRl GCP(https://celo.stake.id/#/proposal/49) for reference
   */
  function proposal_configureCINRConstitutionParameters() private {
  ...
  /**
   * @notice adds cINR token to the main reserve
   */
  function proposal_addCINRToReserve() private {
    if (IReserve(reserveProxy).isStableAsset(stableTokenINRProxy) == false) {
      transactions.push(
        ICeloGovernance.Transaction(
          0,
          reserveProxy,
          abi.encodeWithSelector(IReserve(0).addToken.selector, stableTokenINRProxy)
        )
      );
    } else {
      console.log("Token already added to the reserve, skipping: %s", stableTokenINRProxy);
    }
  }

  /**
   * @notice enable gas payments with cINR
   */
  function proposal_enableGasPaymentsWithCINR() private {
    ...
    for (uint256 i = 0; i < whitelist.length; i++) {
      if (whitelist[i] == stableTokenINRProxy) {
        console.log("Gas payments with cINR already enabled, skipping");
        return;
      }
    }
    transactions.push(
      ICeloGovernance.Transaction(
        ...
        abi.encodeWithSelector(IFeeCurrencyWhitelist(0).addToken.selector, stableTokenINRProxy)
      )
    );
  }
   ...
   */
  function proposal_createExchange(cINRConfig.cINR memory config) private {
   ...
   */
  function proposal_configureTradingLimits(cINRConfig.cINR memory config) private {
    bytes32 exchangeId = keccak256(
      abi.encodePacked("cUSD", "cINR", config.poolConfig.isConstantSum ? "ConstantSum" : "ConstantProduct")
    );
   ...
   */
  function proposal_configureBreakerBox(cINRConfig.cINR memory config) private {
   ...
   */
  function proposal_configureMedianDeltaBreaker(cINRConfig.cINR memory config) private {
