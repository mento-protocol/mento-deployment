...
contract cPHPChecksVerify is cPHPChecksBase {
  ...
  function run() public {
    ...
    console.log("\nStarting cPHP checks:");

    console.log("\n==  Rate feeds ==");
    console.log("   PHPUSD: %s", config.rateFeedConfig.rateFeedID);
  ...
  function verifyToken(cPHPConfig.cPHP memory config) internal {
    console.log("\n== Verifying Token Config Transactions ==");
    verifyOwner();
    verifyCPHPStableToken(config);
    verifycPHPAddedToReserve();
    verifycPHPAddedToFeeCurrencyWhitelist();
    verifyConstitution();
  }

  function verifyOwner() internal view {
    require(Proxy(cPHP)._getOwner() == governance, "StableTokenPHP Proxy ownership not transferred to governance");
    console.log("🟢 cPHP proxy ownership transferred to governance");
  }

  function verifyCPHPStableToken(cPHPConfig.cPHP memory config) internal {
    StableTokenPHPProxy stableTokenPHPProxy = StableTokenPHPProxy(cPHP);
    address stableTokenV2 = contracts.deployed("StableTokenV2");

    address cPHPImplementation = stableTokenPHPProxy._getImplementation();
    if (cPHPImplementation != stableTokenV2) {
      console.log(
    ...
    IStableTokenV2 cPHPToken = IStableTokenV2(cPHP);
    IERC20Metadata cPHPTokenMetadata = IERC20Metadata(cPHP);

    assertEq(cPHPTokenMetadata.name(), config.stableTokenConfig.name, "❗️❌ cPHP name not set correctly!");
    assertEq(cPHPTokenMetadata.symbol(), config.stableTokenConfig.symbol, "❗️❌ cPHP symbol not set correctly!");
    assertEq(cPHPToken.broker(), broker, "❗️❌ cPHP broker not set correctly!");
    assertEq(cPHPToken.validators(), validators, "❗️❌ cPHP validators not set correctly!");

    // no pre-mint
    assertEq(cPHPToken.totalSupply(), 0, "❗️❌ cPHP pre-minted tokens!");
  }

  function verifycPHPAddedToReserve() internal view {
    if (!Reserve(address(uint160(reserve))).isStableAsset(cPHP)) {
      revert("cPHP has not been added to the reserve.");
    }

    console.log("🟢 cPHP has been added to the reserve");
  }

  function verifycPHPAddedToFeeCurrencyWhitelist() internal view {
    address[] memory feeCurrencyWhitelist = IFeeCurrencyWhitelist(contracts.celoRegistry("FeeCurrencyWhitelist"))
      .getWhitelist();

    if (!Arrays.contains(feeCurrencyWhitelist, cPHP)) {
      revert("cPHP has not been added to the fee currency whitelist.");
    }

    console.log("🟢 cPHP has been added to the fee currency whitelist");
  }
  ...
  function checkConstitutionParam(bytes4 functionSelector, uint256 expectedValue) internal view {
    uint256 actualConstitutionValue = celoGovernance.getConstitution(cPHP, functionSelector);
  ...
  function verifyExchange(cPHPConfig.cPHP memory config) internal view {
    console.log("\n== Verifying exchanges ==");
  ...
  function verifyPoolExchange(cPHPConfig.cPHP memory config) internal view {
  ...
  function verifyPoolConfig(cPHPConfig.cPHP memory config) internal view {
    bytes32 exchangeId = getExchangeId(
      config.poolConfig.asset0,
      config.poolConfig.asset1,
      config.poolConfig.isConstantSum
  ...
  function verifyTradingLimits(cPHPConfig.cPHP memory config) internal view {
  ...
  function verifyCircuitBreaker(cPHPConfig.cPHP memory config) internal view {
  ...
  function verifyBreakersAreEnabled(cPHPConfig.cPHP memory config) internal view {
  ...
  function verifyMedianDeltaBreaker(cPHPConfig.cPHP memory config) internal view {
