...
contract FX02_Deploy_ChainlinkRelayers is Script {
  ...
  Relayer[] private relayers = [
    Relayer({
      rateFeed: "relayed:CELOJPY",
      rateFeedDescription: "CELO/JPY (CELO/USD:USD/JPY)",
      ...
      aggregators: aggregators(
        ...
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.JPYUSD"), invert: true })
      )
    }),
    Relayer({
      rateFeed: "relayed:JPYUSD",
      rateFeedDescription: "JPY/USD",
      maxTimestampSpread: 0,
      aggregators: aggregators(
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.JPYUSD"), invert: false })
      )
    }),
    Relayer({
      rateFeed: "relayed:CELONGN",
      rateFeedDescription: "CELO/NGN (CELO/USD:USD/NGN)",
      maxTimestampSpread: 24 hours,
      aggregators: aggregators(
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.CELOUSD"), invert: false }),
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.NGNUSD"), invert: true })
      )
    }),
    Relayer({
      rateFeed: "relayed:NGNUSD",
      rateFeedDescription: "NGN/USD",
      maxTimestampSpread: 0,
      aggregators: aggregators(
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.NGNUSD"), invert: false })
      )
    })
  ];
