...
import { FX03ChecksBase } from "./FX03Checks.base.sol";
...
contract FX03ChecksVerify is FX03ChecksBase {
  ...
  function run() public {
    ...
    console.log("\nStarting FX03 Checks for Celo Governance Proposal:");

    console.log("\n==  Rate feeds ==");
    console.log("   CHFUSD: %s", config.CHFUSD.rateFeedID);
    ...
    verifyStableToken(cCHF, config.cCHFConfig);
  ...
  function verifyOwner() internal view {
    require(Proxy(cCHF)._getOwner() == governance, "StableTokenCHF Proxy ownership not transferred to governance");
    console.log("🟢 cCHF proxy ownership transferred to governance");

    require(Proxy(cNGN)._getOwner() == governance, "StableTokenNGN Proxy ownership not transferred to governance");
    console.log("🟢 cNGN proxy ownership transferred to governance");
  ...
  function verifyTokensAddedToReserve() internal view {
    if (!Reserve(address(uint160(reserve))).isStableAsset(cCHF)) {
      revert("cCHF has not been added to the reserve.");
    }
    console.log("🟢 cCHF has been added to the reserve");
  ...
  function verifyTokensAddedToFeeCurrencyDirectory() internal view {
    ...
    if (!Arrays.contains(feeCurrencies, cCHF)) {
      revert("cCHF has not been added to the fee currency whitelist.");
    }
    console.log("🟢 cCHF has been added to the fee currency whitelist");
  ...
  function verifyConstitution() internal view {
    ...
    for (uint256 i = 0; i < constitutionFunctionSelectors.length; i++) {
      ...
      checkConstitutionParam(cCHF, selector, expectedValue);
