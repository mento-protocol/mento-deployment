...
contract ETHRelayer_Deploy_ChainlinkRelayers is Script {
  ...
  Relayer[] private relayers = [
    Relayer({
      ...
      rateFeedDescription: "CELO/ETH (CELO/USD:USD/ETH)",
      ...
      aggregators: aggregators(
        ...
        IChainlinkRelayer.ChainlinkAggregator({ aggregator: contracts.dependency("Chainlink.ETHUSD"), invert: true })
      )
    })
  ];
  ...
  function run() public {
    ...
    {
      deployRelayerIfNoneOrDifferent(relayers[0]);
    }
    vm.stopBroadcast();

    address rateFeedId = toRateFeedId(relayers[0].rateFeed);
    address relayer = relayerFactory.getRelayer(rateFeedId);
    console.log("%s - ratefeed:%s, relayer:%s", relayers[0].rateFeed, rateFeedId, relayer);
  }
