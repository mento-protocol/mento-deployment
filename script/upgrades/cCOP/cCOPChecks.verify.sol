...
contract cCOPChecksVerify is cCOPChecksBase {
  ...
  function verifyToken(cCOPConfig.cCOP memory config) internal {
    ...
    verifyOwner();
    verifyCCOPStableToken(config);
    verifycCOPAddedToReserve();
    verifycCOPAddedToFeeCurrencyWhitelist();
    verifyConstitution();
  }
  ...
  function verifyCCOPStableToken(cCOPConfig.cCOP memory config) internal {
    ...
    if (cCOPImplementation != stableTokenV2) {
      console.log(
        "The implementation from StableTokenCOPProxy: %s does not match the deployed StableTokenV2 address: %s.",
        cCOPImplementation,
        stableTokenV2
      );
      revert("StableTokenCOPProxy does not points to StableTokenV2 deployed implementation. See logs.");
    }
  ...
  function verifycCOPAddedToReserve() internal view {
  ...
  function verifycCOPAddedToFeeCurrencyWhitelist() internal view {
